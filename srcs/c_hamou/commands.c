...
/*   Updated: 2025/05/29 01:41:18 by oait-si-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// {
//     t_command *command = gc_malloc(head, sizeof(t_command));
//     if (!command)
//         return (NULL);
//     command->cmd = NULL;
//     command->args = NULL;
...
// static int populate_command(t_head_list *head,t_command *cmd, t_token *tokens, int arg_c, int in_c, int out_c)
// {
//     int i = 0, j = 0, k = 0, append_idx = 0;

//     cmd->args = gc_malloc(head,sizeof(char *) * (arg_c + 1));
//     cmd->red_in = gc_malloc(head,sizeof(char *) * (in_c + 1));
//     cmd->red_out = gc_malloc(head,sizeof(char *) * (out_c + 1));
//     cmd->append = gc_malloc(head,sizeof(int) * out_c);
//     if (!cmd->args || !cmd->red_in || !cmd->red_out || (out_c && !cmd->append))
//         return (0);
//     while (tokens && tokens->type != TOKEN_PIPE)
//     {
...
//         {
//             if (!i)
//                 cmd->cmd = ft_strdup(head,tokens->value);
//             cmd->args[i++] = ft_strdup(head,tokens->value);
//         }
//         else if (tokens->type == TOKEN_RED_IN && tokens->next)
//         {
//             tokens = tokens->next;
//             cmd->red_in[j++] = ft_strdup(head,tokens->value);
//         }
//         else if ((tokens->type == TOKEN_RED_OUT || tokens->type == TOKEN_RED_APPEND) && tokens->next)
//         {
//             cmd->append[append_idx] = (tokens->type == TOKEN_RED_APPEND);
//             tokens = tokens->next;
//             cmd->red_out[k++] = ft_strdup(head,tokens->value);
//             append_idx++;
//         }
//         else if (tokens->type == TOKEN_RED_HEREDOC && tokens->next)
//         {
//             tokens = tokens->next;
//             cmd->heredoc_delimiter = ft_strdup(head,tokens->value);
//         }
//         tokens = tokens->next;
//     }
//     cmd->args[i] = NULL;
...
//         {
//             if (arg_count || red_in_count || red_out_count)
//                 if (!populate_command(head ,current, tokens, arg_count, red_in_count, red_out_count))
//                     return (free_gc(head), NULL);
//             current = NULL;
//             arg_count = 0;
//             red_in_count = 0;
